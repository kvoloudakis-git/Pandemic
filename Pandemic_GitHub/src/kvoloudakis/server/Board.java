...
public class Board implements Serializable, Cloneable{
	...
	public void rolesSetup(int rolesSetup)
	{
		System.out.println("Setting up fixed roles!!");
		
		if (rolesSetup == 1)
		{
			roles[0] = "Medic";
			roles[1] = "Operations Expert";
			roles[2] = "Scientist";
			roles[3] = "Quarantine Specialist";
		}
		else if (rolesSetup == 2)
		{
			roles[1] = "Medic";
			roles[2] = "Operations Expert";
			roles[3] = "Scientist";
			roles[0] = "Quarantine Specialist";
		}
		else if (rolesSetup == 3)
		{
			roles[2] = "Medic";
			roles[3] = "Operations Expert";
			roles[0] = "Scientist";
			roles[1] = "Quarantine Specialist";
		}
		else if (rolesSetup == 4)
		{
			roles[3] = "Medic";
			roles[0] = "Operations Expert";
			roles[1] = "Scientist";
			roles[2] = "Quarantine Specialist";
		}
	...
	public void deckSetup(int cardsSetup)
	{
		initializeCities();
		
		if (cardsSetup == 0)
		{
			System.out.println("\nSetting up random deck with " + numberOfEpidemicCards + " epidemics!!\n");
			
			initializeInfectedDeck();
			initializePlayersDeck();
			initializeGameVariables();
			initializePawnsPositions();
			manualBuildResearchStation("Atlanta");
			initializePlayersHands();
			addEpidemicCards();
			
		}
		else if (cardsSetup == 1)
		{
			System.out.println("\nSetting up fixed deck 1 with " + numberOfEpidemicCards + " epidemics!!\n");
			deck1(numberOfEpidemicCards);
		}
		else if (cardsSetup == 2)
		{
			System.out.println("\nSetting up fixed deck 2 with " + numberOfEpidemicCards + " epidemics!!\n");
			deck2(numberOfEpidemicCards);
		}
		else if (cardsSetup == 3)
		{
			System.out.println("\nSetting up fixed deck 3 with " + numberOfEpidemicCards + " epidemics!!\n");
			deck3(numberOfEpidemicCards);
		}
		
	}
	
	// Just shuffling the roles for players to get randomly
	public void initializeRoles(boolean fixedRoles)
	{
		if (fixedRoles)
		{
			roles[0] = "Medic";
	...
	// Adding Event Cards to Players Deck and shuffling
	public void addEventCards()
	{
		System.out.println("Adding " + numberOfEventCards + " Event cards to the player Deck..\n");
		
		int i = 0;
		
		while (true)
		{
			if (i == numberOfEventCards)
				break;
			
			playersDeck.add("Resilient Population");
			i++;
			
			if (i == numberOfEventCards)
				break;
			playersDeck.add("One Quiet Night");
			i++;
			
			if (i == numberOfEventCards)
				break;
			playersDeck.add("Forecast");
			i++;
			
			if (i == numberOfEventCards)
				break;
			playersDeck.add("Airlift");
			i++;
			
			if (i == numberOfEventCards)
				break;
			playersDeck.add("Government Grant");
			i++;
	...
	// "Dungeon Master" Actions (aka actions that are by the game itself)
	...
	public void addCubes(String cityToAddCubes, int numberOfCubes, String cubeColor)
	{
		...
		if (outbreak == true && gameEnded == false && !this.canNotOutbreak.contains(cityList.get(indexOfCity).getName())) //here
		{	
			...
			else if (outbreaksCount < outbreaksLimit)
			{
				for (int i = 0 ; i < cityList.get(indexOfCity).getNeighboursNumber() ; i++)
					if (!this.canNotOutbreak.contains(cityList.get(indexOfCity).getNeighbour(i))) //here
		...
		//this.canNotOutbreak.remove(cityList.get(indexOfCity).getName()); // EDWWWWWWWWWWWWWWWWWWWWWW
	}
	...
	public void driveTo(int playerID, String destination)
	{	
	...
	public void directFlight(int playerID, String destination)
	{
		boolean isLegal = false;
	...
	public void charterFlight(int playerID, String destination)
	{
		boolean isLegal = false;
	...
	public void shuttleFlight(int playerID, String destination)
	{
		boolean isLegal = false;
	...
	public void buildRS(int playerID, String cityToBuild)
	{
	...
	public void removeRS(int playerID, String cityToRemove)
	{
	...
	public void treatDisease(int playerID, String cityToTreat, String color)
	{
		City currentCity = searchForCity(getPawnsLocations(playerID));
	...
	public void cureDisease(int playerID, String colorToCure)
	{
	...
	public void cureDisease(int playerID, String colorToCure, String card1, String card2, String card3, String card4, String card5)
	{
		boolean isScientist = getRoleOf(playerID).equals("Scientist");
		int cardsColorCount = 0;
		boolean isLegal = false;
		...
		if (getRSLocations().contains(getPawnsLocations(playerID)))
		{
			...
			if (isLegal)
			{
				...
				if (!isScientist)
					getHandOf(playerID).remove(card5);
	...
	// Give = true & Take = false
	public void shareKnowledge(boolean giveOrTake, String cardName, int myID, int playerIDToSwap)
	{
		if (getPawnsLocations(myID) == getPawnsLocations(playerIDToSwap) && getPawnsLocations(myID) == cardName)
		{
			if (giveOrTake && getHandOf(myID).contains(cardName))
			{
				getHandOf(myID).remove(cardName);
				getHandOf(playerIDToSwap).add(cardName);
				System.out.println("Succesfully swaped " + cardName + " with " + usernames[playerIDToSwap]);
			}
			else if (!giveOrTake && getHandOf(playerIDToSwap).contains(cardName))
			{
				getHandOf(playerIDToSwap).remove(cardName);
				getHandOf(myID).add(cardName);
				System.out.println("Succesfully swaped " + cardName + " with " + usernames[playerIDToSwap]);
			}
			else
				System.out.println("You or the other player don't have the card of " + cardName + " to give it to someone else..");
		}
		else 
			System.out.println("Could not trade " + cardName + ". Must be both in the same city as the card you want to trade..");
	}
	...
	// Government Grant : Building 1 RS wherever you like (no city card needed)
	public void playGovernmentGrant(int myID, String cityToBuild)
	{
		boolean isLegal = false;
		
		if (getHandOf(myID).contains("Government Grant") && RSCount < RSLimit)
			isLegal = true;
		else 
			isLegal = false;
		
		if (isLegal) {
			System.out.println(usernames[myID] + " is building an RS in " + cityToBuild + " using Government Grant card");
			getHandOf(myID).remove("Government Grant");
			RSLocations.add(cityToBuild);
		}
		else
			System.out.println("Could not use Government Grant card to build an RS in " + cityToBuild);
	}
	
	// One Quiet Night : Skip the next infection phase
	public void playOneQuietNight(int myID)
	{
		boolean isLegal = false;
		
		if (getHandOf(myID).contains("One Quiet Night"))
			isLegal = true;
		else 
			isLegal = false;
		
		if (isLegal) {
			System.out.println(usernames[myID] + " is using One Quiet Night card");
			isQuietNight = true;
			getHandOf(myID).remove("One Quiet Night");
		}
		else
			System.out.println("You don't have the event card of One Quiet Night...");
	}
	
	// Airlift : Skip the next infection phase
	public void playAirlift(int myID, int playerToMove, String cityToMoveTo)
	{
		boolean isLegal = false;
		
		if (getHandOf(myID).contains("Airlift"))
			isLegal = true;
		else 
			isLegal = false;
		
		if (isLegal) {
			System.out.println(usernames[myID] + " is using Airlift card to move " + usernames[playerToMove] + " to " + cityToMoveTo);
			pawnsLocations[playerToMove] = cityToMoveTo;
			getHandOf(myID).remove("Airlift");
		}
		else
			System.out.println("You don't have the event card of Airlift...");
	}
	
	// Forecast : Get a sneak peak to the first 6 cards of the infection deck
	public void playForecast(int myID)
	{
		boolean isLegal = false;
		
		if (getHandOf(myID).contains("Forecast"))
			isLegal = true;
		else 
			isLegal = false;
		
		if (isLegal) {
			System.out.println(usernames[myID] + " is using Forecast card to sneak peak to the 6 first cards of the infetcion deck..");
			System.out.println("First to come card : " + infectedDeck.get(0).toString());
			System.out.println("First to come card : " + infectedDeck.get(1).toString());
			System.out.println("First to come card : " + infectedDeck.get(2).toString());
			System.out.println("First to come card : " + infectedDeck.get(3).toString());
			System.out.println("First to come card : " + infectedDeck.get(4).toString());
			System.out.println("First to come card : " + infectedDeck.get(5).toString());
			getHandOf(myID).remove("Forecast");
		}
		else
			System.out.println("You don't have the event card of Forecast...");
	}
	
	// Resilient Population : Get a sneak peak to the first 6 cards of the infection deck
	public void playResilientPopulation(int myID, String cityCardToRemove)
	{
		boolean isLegal = false;
		
		if (getHandOf(myID).contains("Resilient Population") && infectedDeck.contains(cityCardToRemove))
			isLegal = true;
		else 
			isLegal = false;
		
		if (isLegal) {
			System.out.println(usernames[myID] + " is using Resilient Population card to remove " + cityCardToRemove + " from the infection deck..");
			infectedDeck.remove(cityCardToRemove);
			discardedPile.add(cityCardToRemove);
			getHandOf(myID).remove("Resilient Population");
		}
		else
			System.out.println("You don't have the event card of Resilient Population or the card is already in the discarted pile ...");
	}
	...
	public int getOutbreakLimit() {
		return outbreaksLimit;
	}
	
	public int getInitialCubesLimit() {
		return initialCubesLimit;
	}
	
	public void deck3(int epidemicsCount)
	{
		infectedDeck.add("Santiago");
		infectedDeck.add("Washington");
		infectedDeck.add("Kinshasa");
		infectedDeck.add("Milan");
		infectedDeck.add("Los Angeles");
		infectedDeck.add("Lagos");
		infectedDeck.add("Ho Chi Minh City");
		infectedDeck.add("Bangkok");
		infectedDeck.add("Lima");
		infectedDeck.add("Cairo");
		infectedDeck.add("Moscow");
		infectedDeck.add("Chicago");
		infectedDeck.add("San Fransisco");
		infectedDeck.add("Bogota");
		infectedDeck.add("Manila");
		infectedDeck.add("Osaka");
		infectedDeck.add("Tehran");
		infectedDeck.add("Riyadh");
		infectedDeck.add("Paris");
		infectedDeck.add("Delhi");
		infectedDeck.add("Kolkata");
		infectedDeck.add("Karachi");
		infectedDeck.add("Taipei");
		infectedDeck.add("Chennai");
		infectedDeck.add("Baghdad");
		infectedDeck.add("Beijing");
		infectedDeck.add("Montreal");
		infectedDeck.add("Madrid");
		infectedDeck.add("Instabul");
		infectedDeck.add("Algiers");
		infectedDeck.add("Johannesburg");
		infectedDeck.add("Miami");
		infectedDeck.add("Sydney");
		infectedDeck.add("Seoul");
		infectedDeck.add("Hong Kong");
		infectedDeck.add("Khartoum");
		infectedDeck.add("London");
		infectedDeck.add("Atlanta");
		infectedDeck.add("Mumbai");
		infectedDeck.add("St. Petersburg");
		infectedDeck.add("Sao Paulo");
		infectedDeck.add("Mexico City");
		infectedDeck.add("Essen");
		infectedDeck.add("Buenos Aires");
		infectedDeck.add("Jakarta");
		infectedDeck.add("Shanghai");
		infectedDeck.add("Tokyo");
		infectedDeck.add("New York");

		playersDeck.add("Paris");
		playersDeck.add("Beijing");
		playersDeck.add("Essen");
		playersDeck.add("Shanghai");
		playersDeck.add("New York");
		playersDeck.add("Bangkok");
		playersDeck.add("Moscow");
		playersDeck.add("London");
		playersDeck.add("Taipei");
		playersDeck.add("Lagos");
		playersDeck.add("Seoul");
		playersDeck.add("Lima");
		playersDeck.add("Khartoum");
		playersDeck.add("Montreal");
		playersDeck.add("Cairo");
		playersDeck.add("Sao Paulo");
		playersDeck.add("Baghdad");
		playersDeck.add("Bogota");
		playersDeck.add("Milan");
		playersDeck.add("Karachi");
		playersDeck.add("Algiers");
		playersDeck.add("Jakarta");
		playersDeck.add("Delhi");
		playersDeck.add("Osaka");
		playersDeck.add("Atlanta");
		playersDeck.add("Sydney");
		playersDeck.add("Instabul");
		playersDeck.add("St. Petersburg");
		playersDeck.add("Johannesburg");
		playersDeck.add("Tokyo");
		playersDeck.add("Miami");
		playersDeck.add("Manila");
		playersDeck.add("Santiago");
		playersDeck.add("Chicago");
		playersDeck.add("San Fransisco");
		playersDeck.add("Tehran");
		playersDeck.add("Los Angeles");
		playersDeck.add("Hong Kong");
		playersDeck.add("Madrid");
		playersDeck.add("Chennai");
		playersDeck.add("Kinshasa");
		playersDeck.add("Buenos Aires");
		playersDeck.add("Riyadh");
		playersDeck.add("Ho Chi Minh City");
		playersDeck.add("Mexico City");
		playersDeck.add("Mumbai");
		playersDeck.add("Washington");
		playersDeck.add("Kolkata");
	...
	public void deck2(int epidemicsCount)
	{
		infectedDeck.add("Beijing");
		infectedDeck.add("Sao Paulo");
		infectedDeck.add("Atlanta");
	...
	public void deck1(int epidemicsCount)
	{
		infectedDeck.add("Bangkok");
		infectedDeck.add("Karachi");
		infectedDeck.add("Mexico City");
